...
public class AiService {
    ...
    public AiService(ChatClient.Builder chatClientBuilder) {
        ...
        promptTemplates.put("deep_search_ranker_ar", """
أنت مصنِّف مطابقة نتائج بحث.

الغرض:
- تحديد مدى توافق "نتيجة البحث" مع "طلب البحث".

لغة الإخراج:
- أعد **String فقط** بدون أي نص آخر.

صيغة الإخراج المسموح بها (أحدها فقط):
"مطابقة مرتفعة", "مطابقة متوسطة", "مطابقة منخفضة", "لا يوجد مطابقة"

سياسة تقييم أقل تشددًا (خصوصًا للسيارات):
- سيارات (الأهم أولًا):
  1) الماركة + الموديل = العامل الأساسي.
  2) السنة: تُعد مناسبة إن كانت مطابقة أو قريبة (±2 سنوات مقبول).
  3) باقي الحقول (اللون، الوقود، العداد، المدينة، السعر): عوامل مُساعدة لا تُسقِط المطابقة القوية عند اختلافها البسيط.
- عقار:
  - النوع (شقة/فيلا..)، إيجار/بيع، المدينة/الحي عوامل أساسية؛ المساحة/الغرف/الدورات والسعر عوامل مساعدة. غياب بعض الحقول لا يُعد سلبًا قويًا.

قواعد اتخاذ القرار (إرشادية مرنة):
- "مطابقة مرتفعة":
  - سيارات: تطابق الماركة والموديل، والسنة مطابقة أو قريبة (±2)، حتى لو اختلفت سمات ثانوية.
  - عقار: تطابق نوع الاستخدام (إيجار/بيع) والنوع والموقع العام، مع توافق مقبول في المساحة/الغرف أو السعر.
- "مطابقة متوسطة":
  - سيارات: تطابق الماركة والموديل مع سنة أبعد قليلًا أو معلومات ناقصة، أو تطابق قوي في أحدهما مع قرب جيّد في الآخر.
  - عقار: تطابق جيّد في النوع/الاستخدام أو الموقع مع فروقات ملحوظة في مساحة/سعر/غرف.
- "مطابقة منخفضة":
  - سيارات: تطابق ضعيف (مثل تطابق الماركة فقط دون الموديل) أو سنة بعيدة مع نقص واضح في العوامل المساعدة.
  - عقار: تطابق جزئي في نوع أو موقع مع فروقات جوهرية أخرى.
- "لا يوجد مطابقة":
  - لا يوجد أي تقاطع مهم في العوامل الأساسية (مثلاً موديل مختلف تمامًا أو نوع عقار مختلف كليًا عن المطلوب).

القرب العددي:
- عامل تحسيني فقط (السنة/المساحة/العداد/السعر)، لا يُستخدم لإقصاء تطابق أساسي واضح.

مدخلاتك (سيتم حقنها كما هي):
...
أعد الآن **String فقط** بهذا الشكل (مثال):
"مطابقة متوسطة"
""");
